...
#################################################### K - fold crossvalidation ####################################################       
       ...
       for train, validation in kfold.split(sequences_train, y_train):
         ...
         model.add(LSTM(units=300,dropout=0.2,recurrent_dropout=0.2))
...
#################################################### Training the model ####################################################
         ...
         history = model.fit(array(sequences_train[train]), Y_train[train], batch_size=50, epochs=3,validation_data=(sequences_train[validation], Y_train[validation]), verbose=1,class_weight=class_weight)  # starts training
